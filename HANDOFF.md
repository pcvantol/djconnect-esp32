...
- Release and CI build flows run `scripts/update_build_dependencies.sh` before compiling firmware. It upgrades PlatformIO Core, updates global/project PlatformIO packages, dependencies and tools for the LilyGO environment, writes a dependency diff so `THIRD_PARTY_NOTICES.md` plus `DESIGN_DECISIONS.md` can be refreshed whenever frameworks, libraries or tools changed, and publishes GitHub release notes from the matching `CHANGELOG.md` version section.
