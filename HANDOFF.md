...
- The boot screen shows `Muziekbediening met karakter` and `https://djconnect.pages.dev` for at least three seconds; the About screen shows the website URL and no longer includes a separate `Firmware / Proprietary` row. WiFi boot connection timeout is 15 seconds. During WiFi connect, the LED ring shows a green chase animation.
