...
- Home Assistant is authoritative for pairing validity. HA 401/403/404 responses, or HA errors such as `not_configured`/`stale_pairing`, clear established local pairing and return the ESP to pairing mode. Playback proxy commands are disabled until a successful authenticated HA status post confirms the stored token.
