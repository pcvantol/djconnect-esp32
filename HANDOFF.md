...
- Latest firmware release target from this repo: `v3.1.16`. Source repo `pcvantol/djconnect-esp32` and public firmware repo `pcvantol/djconnect-firmware` have been cleaned so the remote stable semver tag/release set keeps only `v3.1.16`; generated `release/` artifacts are ignored in source.
...
- Web portal includes Now Playing, DJ-announcement simulation, outputs, playlists, Up Next with per-item play, refresh and lazy browser-loaded album-art thumbnails, local browser games, logs, diagnostics, OTA upload, WiFi update, settings and status indicators. It uses the DJConnect blue/purple brand styling, shows `Muziekbediening met karakter`, firmware version plus board device model in the title bar, uses lila/magenta playback controls and volume slider, and hides the battery icon on boards without a battery gauge such as ESP32-S3-BOX-3. Up Next supports up to 20 queue items from Home Assistant and is de-duplicated by URI or title/subtitle fallback so single-item queues do not render repeated tracks. Device logs are scrollable with the encoder and use compact `HH:mm INF` prefixes.
...
The native logic test, release-script test and local dual-board release build passed for `v3.1.16`. The public v3.1.16 release publishes both `djconnect-lilygo-t-embed-s3-v3.1.16.bin` and `djconnect-esp32-s3-box-3-v3.1.16.bin`, both `.sha256` files and `firmware_manifest.json` to `pcvantol/djconnect-firmware`.
...
8. Re-test OTA from a no-PSRAM LilyGO on `v3.1.16` to confirm the GitHub TLS memory issue is gone.
