...
The native logic test, release-script test and local LilyGO/BOX-3 builds passed during `v3.1.29` preparation. The release flow builds both `djconnect-lilygo-t-embed-s3-v3.1.29.bin` and `djconnect-esp32-s3-box-3-v3.1.29.bin`, both `.sha256` files and `firmware_manifest.json` for `pcvantol/djconnect-firmware`.
