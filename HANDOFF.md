...
- Latest firmware release target from this repo: `v3.1.11`. Source repo `pcvantol/djconnect-esp32` and public firmware repo `pcvantol/djconnect-firmware` have been cleaned so the remote stable semver tag/release set keeps only `v3.1.11`; generated `release/` artifacts are ignored in source.
...
All passed before the v3.1.11 release. The public v3.1.11 release publishes both `djconnect-lilygo-t-embed-s3-v3.1.11.bin` and `djconnect-esp32-s3-box-3-v3.1.11.bin`, both `.sha256` files and `firmware_manifest.json` to `pcvantol/djconnect-firmware`.
...
8. Re-test OTA from a no-PSRAM LilyGO on `v3.1.11` to confirm the GitHub TLS memory issue is gone.
