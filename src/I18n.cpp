...
constexpr const char *SupportedLanguageCodes[I18n::SupportedLanguageCount] = {"en", "nl", "de", "fr", "es"};
...
constexpr Entry Entries[] = {
    ...
    {"firmware_update_progress", {"Firmware update\nin progress..", "Firmware update\nbezig..", "Firmware-Update\nlaeuft..", "Firmware update\nin progress..", "Firmware update\nin progress.."}},
    {"flyer", {"Sky Dash", "Sky Dash", "Sky Dash", "Sky Dash", "Sky Dash"}},
    {"games", {"Games", "Games", "Spiele", "Jeux", "Juegos"}},
    {"ha_pairing_invalid", {"Home Assistant pairing invalid. Reset pairing and pair again.", "Home Assistant koppeling ongeldig. Reset de koppeling en koppel opnieuw.", "HA-Kopplung ungueltig. Neu koppeln.", "Home Assistant pairing invalid. Reset pairing and pair again.", "Home Assistant pairing invalid. Reset pairing and pair again."}},
    {"help", {"Help", "Help", "Hilfe", "Aide", "Ayuda"}},
    {"help_top_short", {"Top short: back / next", "Boven kort: terug / volgende", "Oben kurz: zurueck/weiter", "Top short: back / next", "Top short: back / next"}},
    {"help_top_double", {"Top double: previous", "Boven dubbel: vorige", "Oben doppelt: vorheriger", "Top double: previous", "Top double: previous"}},
    {"help_top_hold", {"Top hold: menu", "Boven vasthouden: menu", "Oben halten: Menue", "Top hold: menu", "Top hold: menu"}},
    {"help_top_10s", {"Top 10s: restart", "Boven 10s: herstart", "Oben 10s: Neustart", "Top 10s: restart", "Top 10s: restart"}},
    {"help_center_short", {"Center short: play/pause", "Midden kort: play/pauze", "Mitte kurz: Play/Pause", "Center short: play/pause", "Center short: play/pause"}},
    {"help_center_hold", {"Center hold: voice PTT", "Midden vasthouden: voice PTT", "Mitte halten: Voice PTT", "Center hold: voice PTT", "Center hold: voice PTT"}},
    {"help_encoder_turn", {"Encoder: volume / scroll", "Encoder: volume / scroll", "Encoder: Lautst./Scroll", "Encoder: volume / scroll", "Encoder: volume / scroll"}},
    {"help_games", {"Games: center fires", "Games: midden schiet", "Spiele: Mitte feuert", "Games: center fires", "Games: center fires"}},
    {"center_back", {"Center press = back", "Terug = middenknop", "Mitte = zurueck", "Center press = back", "Center press = back"}},
    {"center_liked_proxy", {"Default playlist = center", "Standaard playlist = middenknop", "Standardliste = Mitte", "Default playlist = center", "Default playlist = center"}},
    ...
    {"log_level", {"Log level", "Logniveau", "Logstufe", "Log level", "Log level"}},
    ...
    {"liked_proxy_started", {"Default playlist started", "Standaard playlist gestart", "Standardliste gestartet", "Default playlist started", "Default playlist started"}},
    {"liked_proxy_not_found", {"Default playlist not found", "Standaard playlist niet gevonden", "Standardliste nicht gefunden", "Default playlist not found", "Default playlist not found"}},
    {"loading_next_track", {"Loading next track", "Volgende nummer laden", "Naechsten Titel laden", "Loading next track", "Loading next track"}},
    {"loading_outputs", {"Loading outputs", "Geluidsuitgangen laden", "Ausgaenge laden", "Loading outputs", "Loading outputs"}},
    {"loading_playlists", {"Loading playlists", "Afspeellijsten laden", "Playlists laden", "Loading playlists", "Loading playlists"}},
    {"loading_previous_track", {"Loading previous track", "Vorige nummer laden", "Vorigen Titel laden", "Loading previous track", "Loading previous track"}},
    {"loading_queue", {"Loading queue", "Wachtrij laden", "Warteschlange laden", "Loading queue", "Loading queue"}},
    ...
    {"playback_credentials_unavailable", {"Playback credentials unavailable", "Afspeelgegevens niet beschikbaar", "Wiedergabedaten fehlen", "Playback credentials unavailable", "Playback credentials unavailable"}},
    ...
    {"reset_pairing_title", {"Reset Home Assistant pairing?", "Home Assistant koppeling resetten?", "HA-Kopplung resetten?", "Reset Home Assistant pairing?", "Reset Home Assistant pairing?"}},
    ...
    {"setup_success_restart", {"Setup successful. Restarting into normal mode...", "Setup gelukt. Herstarten naar normale modus...", "Setup OK. Neustart...", "Setup successful. Restarting into normal mode...", "Setup successful. Restarting into normal mode..."}},
    ...
    {"battery_ok_restart", {"Battery OK\nRestarting...", "Batterij OK\nHerstarten...", "Akku OK\nNeustart...", "Battery OK\nRestarting...", "Battery OK\nRestarting..."}},
    {"wifi_ok_restart", {"WiFi OK. Restarting...", "WiFi OK. Herstarten...", "WiFi OK. Neustart...", "WiFi OK. Restarting...", "WiFi OK. Restarting..."}},
    ...
    {"confirm_yes_reset_pairing", {"Yes, reset pairing", "Ja, koppeling resetten", "Ja, Kopplung resetten", "Yes, reset pairing", "Yes, reset pairing"}},
    {"confirm_yes_change_wifi", {"Yes, open WiFi portal", "Ja, WiFi portal openen", "Ja, WiFi-Portal", "Yes, open WiFi portal", "Yes, open WiFi portal"}},
    {"confirm_yes_wipe_setup", {"Yes, wipe", "Ja, wissen", "Ja, loeschen", "Yes, wipe", "Yes, wipe"}},
    ...
    {"shuffle_off", {"Shuffle off", "Shuffle uit", "Shuffle aus", "Shuffle off", "Shuffle off"}},
    {"shuffle_on", {"Shuffle on", "Shuffle aan", "Shuffle ein", "Shuffle on", "Shuffle on"}},
    {"repeat", {"Repeat", "Herhalen", "Wiederholen", "Repeter", "Repetir"}},
    {"repeat_off", {"Repeat off", "Herhalen uit", "Wiederholen aus", "Repeat off", "Repeat off"}},
    {"repeat_once", {"Repeat once", "Eenmaal herhalen", "Einmal wiederholen", "Repeat once", "Repeat once"}},
    {"repeat_infinite", {"Repeat infinite", "Oneindig herhalen", "Endlos wiederholen", "Repeat infinite", "Repeat infinite"}},
    ...
    {"spotify_authorization_failed", {"Playback authorization failed", "Afspeelautorisatie mislukt", "Wiedergabe-Autorisierung fehlgeschlagen", "Playback authorization failed", "Playback authorization failed"}},
    {"spotify_authorization_failed_sentence", {"Playback authorization failed.", "Afspeelautorisatie mislukt.", "Wiedergabe-Autorisierung fehlgeschlagen.", "Playback authorization failed.", "Playback authorization failed."}},
    {"spotify_client_and_refresh_required", {"Playback credentials are managed in Home Assistant", "Afspeelgegevens worden beheerd in Home Assistant", "Wiedergabedaten in Home Assistant", "Playback credentials are managed in Home Assistant", "Playback credentials are managed in Home Assistant"}},
    ...
    {"playback_backend_unavailable_hint", {"Check the Spotify connection in Home Assistant.", "Controleer de Spotify-koppeling in Home Assistant.", "Spotify in Home Assistant pruefen.", "Check the Spotify connection in Home Assistant.", "Check the Spotify connection in Home Assistant."}},
    {"spotify_refresh_saved_ok", {"Playback credentials saved", "Afspeelgegevens opgeslagen", "Wiedergabedaten gespeichert", "Playback credentials saved", "Playback credentials saved"}},
    {"spotify_select_output_first", {"Select an output first", "Selecteer eerst een output", "Erst Ausgang waehlen", "Select an output first", "Select an output first"}},
    {"output_switched", {"Output switched", "Output gewisseld", "Ausgang gewechselt", "Sortie changee", "Salida cambiada"}},
    {"starting_liked_proxy", {"Starting default playlist", "Standaard playlist starten", "Standardliste starten", "Starting default playlist", "Starting default playlist"}},
    {"starting_output", {"Starting", "Starten", "Starten", "Demarrage", "Iniciando"}},
    {"starting_playlist", {"Starting playlist", "Afspeellijst starten", "Playlist starten", "Starting playlist", "Starting playlist"}},
    {"stress_test", {"Stress test", "Stresstest", "Stresstest", "Test charge", "Prueba de carga"}},
    {"stress_test_started", {"Stress test started", "Stresstest gestart", "Stresstest gestartet", "Stress test started", "Stress test started"}},
    {"stress_test_stopped", {"Stress test stopped", "Stresstest gestopt", "Stresstest gestoppt", "Stress test stopped", "Stress test stopped"}},
    {"testing_wifi", {"Testing WiFi", "WiFi testen", "Testing WiFi", "Testing WiFi", "Testing WiFi"}},
    {"voice_connecting", {"Connecting...", "Verbinden...", "Verbinden...", "Connexion...", "Conectando..."}},
    {"voice_ha_auth_failed", {"HA authorization failed. Reset pairing and pair again.", "HA autorisatie mislukt. Reset de koppeling en koppel opnieuw.", "HA-Autorisierung fehlgeschlagen. Neu koppeln.", "HA authorization failed. Reset pairing and pair again.", "HA authorization failed. Reset pairing and pair again."}},
    {"voice_ha_endpoint_missing", {"HA voice endpoint not found. Reset pairing and set up the DJConnect integration again.", "HA voice endpoint niet gevonden. Reset de koppeling en stel de DJConnect integratie opnieuw in.", "HA-Voice-Endpunkt fehlt. Neu koppeln.", "HA voice endpoint not found. Reset pairing and set up the DJConnect integration again.", "HA voice endpoint not found. Reset pairing and set up the DJConnect integration again."}},
    ...
    {"voice_response_audio_failed", {"Voice response audio failed", "Voice response audio mislukt", "Voice-Audio fehlgeschlagen", "Voice response audio failed", "Voice response audio failed"}},
    {"voice_response_played", {"Voice response played", "Voice response afgespeeld", "Voice-Audio abgespielt", "Voice response played", "Voice response played"}},
    {"web_settings_saved", {"Web settings saved", "Webinstellingen opgeslagen", "Web-Einstellungen gespeichert", "Web settings saved", "Web settings saved"}},
    ...
    {"wifi_test_failed", {"WiFi test failed", "WiFi test mislukt", "WiFi-Test fehlgeschlagen", "WiFi test failed", "WiFi test failed"}},
    ...
    {"setup_portal_active_10m", {"Setup mode active for 10 minutes", "Setup modus actief voor 10 minuten", "Setup 10 Minuten aktiv", "Setup mode active for 10 minutes", "Setup mode active for 10 minutes"}},
...
bool I18n::isSupportedLanguageCode(const String &code) {
  ...
  for (size_t index = 0; index < SupportedLanguageCount; index++) {
    if (normalized == SupportedLanguageCodes[index]) {
      return true;
    }
  }
  return false;
...
const char *I18n::languageCode(Language language) {
  return SupportedLanguageCodes[languageIndex(language)];
}
